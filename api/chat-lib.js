...
async function callLLM(prompt, bodyModel, bodyPlatform) {
  ...
  if (!apiKey) {
    throw new Error(
      `尚未配置 ${pf.label} 的 API Key。请在 Vercel → Settings → Environment Variables 中添加 ${pf.keyEnv}，然后重新部署。`
    );
  }

  // Vercel 函数最长 300s，LLM 生成空间充足：
  // 非推理模型（GLM-4.7-Flash 等）→ 4096 token；推理模型（GLM-Z1 等较慢）→ 2200 token。
  // 内部 AbortSignal 120s 兜底防挂起；超长回答触发截断标记，前端提示用户继续追问。
  ...
  // 带退避重试的请求（最多 1 次；整体受 AbortSignal 120s 限制，Vercel 300s 时限内一定返回）
  let resp = null;
  try {
    ...
    if (e.name === 'TimeoutError' || e.name === 'AbortError') {
      throw new Error('LLM 生成超时（120秒限制）。请把问题拆分成更小步骤提问，或在下方切换更快的免费模型。');
    }
    throw e;
  }

  // 限流重试仍失败：自动尝试同平台备选免费模型（仅智谱开放平台有多个免费模型；只试 1 个最快常用的，带 30s 超时防止挂起）
